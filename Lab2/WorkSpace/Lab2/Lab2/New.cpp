...
void printFiles(FILE * fat12, struct RootEntry* rootEntry_ptr,FileNode* rootnode);	//打印文件名，这个函数在打印目录时会调用下面的printChildren
...
int main() {
	...
	//打印文件名
	printFiles(fat12, rootEntry_ptr,rootnode);
...
void printFiles(FILE * fat12, struct RootEntry* rootEntry_ptr,FileNode* rootnode) {
	...
	for (i = 0; i < RootEntCnt; i++) {
		...
		if ((rootEntry_ptr->DIR_Attr & 0x10) == 0) {
			...
			subNode->numofDic = 0;
			subNode->numoffile = 0;
			rootnode->numoffile = rootnode->numoffile + 1;
			...
			//目录
			subNode->type = 1;
			rootnode->numofDic = rootnode->numofDic + 1;
