...
int cur_org;
	...
	//edwin:
		...
		//后面的填充1
		u8* p ;//= (u8*)(V_MEM_BASE+curCon->original_addr*2);
	...
	case '\b':
		...
		//edwin:
		p = (u8*)(V_MEM_BASE+p_con->cursor*2);
		if(*(p-2)==0){//上一个是0，当前光标是在行首还是非行首，行首就认为是\n(其实也可能是tab)，非行首就是tab          (有一点小问题)
			cur_org = p_con->cursor - p_con->original_addr;
			if(cur_org%80==0){
				while(*(p-2)==0){
					count++;
